...
@job(queue='default')
def create_source_lookup_schedule_task(source_id, name, source_type, attrs):
  ...
  source_lookup_schedule_task.delay(source_id, name, source_type, attrs)
  pass


@job(queue='default', schedule=periodic(days=1, start_date=timezone.now() + relativedelta(days=1)))
def source_lookup_schedule_task(source_id, name, source_type, attrs):
  # use tiger code to create periodic task
  # this task is going to call a service --> and that service will interact w/ the source to get new data
  pass
